      ...
      <div>
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div>
              <h3 className="mb-3 flex items-center gap-2 text-lg font-bold text-[#f5f5f0]">
                <span className="relative flex h-6 w-6 items-center justify-center">
                  ...
                  >
                    <svg className="h-6 w-6 text-[#f5f5f0]" fill="currentColor" viewBox="0 0 24 24">
                  ...
                  >
                    <img
                      src="/hevy-logo.png"
                      alt="Hevy Logo"
                      className="h-6 w-6 rounded-full border border-[#404040]/10 object-cover invert"
                    />
                  </span>
                </span>
                Activity Dashboard
              ...
              <p className="text-xs text-[#f5f5f0]/60">
                ...
                <a
                  ...
                  className="underline decoration-dotted transition-colors hover:text-[#f5f5f0]/80"
                >
                  Strava
                </a>{' '}
                and{' '}
        ...
        <div className="overflow-hidden rounded-lg border border-[#404040]/40">
          <div className="grid grid-cols-1 gap-0 md:grid-cols-2">
            {/* Strava Summary */}
            <div className="flex flex-col justify-center border-b border-[#404040]/30 p-3 sm:p-4 md:border-b-0 md:border-r md:border-r-[#404040]/30">
              <div className="mb-1 flex items-center gap-2">
                <span className="rounded-lg border border-[#404040]/10 px-2 py-0.5 text-xs font-semibold text-[#f5f5f0]/90">
                  Lifetime Cardio Totals
                </span>
              </div>
              <div className="grid grid-cols-3 divide-x divide-[#404040]/30">
                <div className="px-1 text-center sm:px-2">
                  <div className="text-lg font-bold text-[#f5f5f0] sm:text-2xl">
                    {stravaStats?.all_run_totals.distance
                      ? (stravaStats.all_run_totals.distance * 0.000621371).toFixed(0)
                      : '--'}
                  </div>
                  <div className="mt-1 text-xs text-[#f5f5f0]/60 sm:text-sm">miles ran</div>
                </div>
                <div className="px-1 text-center sm:px-2">
                  <div className="text-lg font-bold text-[#f5f5f0] sm:text-2xl">
                  ...
                  <div className="mt-1 text-xs text-[#f5f5f0]/60 sm:text-sm">miles cycled</div>
                </div>
                <div className="px-1 text-center sm:px-2">
                  <div className="text-lg font-bold text-[#f5f5f0] sm:text-2xl">
                  ...
                  <div className="mt-1 text-xs text-[#f5f5f0]/60 sm:text-sm">hours active</div>
                </div>
              </div>
              <div className="mt-4">
                <div className="mb-2 flex items-center gap-2">
                  <svg className="h-4 w-4 text-[#f5f5f0]" fill="currentColor" viewBox="0 0 24 24">
                    <path d='M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z' />
                  </svg>
                  <span className="text-xs font-medium uppercase tracking-wide text-[#f5f5f0]/60 sm:text-sm">
                    Athletic Achievements
                  </span>
                </div>
                <div className="flex h-[60px] items-center justify-between rounded-xl border border-[#404040]/40 p-2 sm:h-[70px] sm:p-3">
                    ...
                    {pastAchievements.map((achievement, index) => (
                      ...
                      >
                        <div className="min-w-0 flex-1">
                          <div className="truncate text-xs font-semibold text-[#f5f5f0] sm:text-sm">
                            {achievement.name}
                          </div>
                          <div className="mt-0.5 text-xs text-[#f5f5f0]/60">
                            ...
                            <button
                              ...
                              className={`h-1.5 w-1.5 rounded-full transition-all duration-300 ${
                                dotIndex === currentAchievementIndex
                                  ? 'scale-125 bg-[#f5f5f0]'
                                  : 'bg-[#f5f5f0]/40 hover:bg-[#f5f5f0]/60'
                              }`}
            ...
            <div className="flex flex-col justify-center p-3 sm:p-4">
              ...
              <div className="mb-3 flex flex-col gap-1 sm:flex-row sm:items-center sm:gap-3">
                <span
                  className="truncate text-lg font-bold text-[#f5f5f0] sm:text-xl"
                  title={hevyStats?.workouts?.[0]?.title || ''}
                >
                  {hevyStats?.workouts?.[0]?.title || 'No Workout'}
                </span>
                {hevyStats?.workouts?.[0]?.location && (
                  <span className="rounded border border-[#404040]/20 px-2 py-0.5 text-xs text-[#f5f5f0]/60">
                    {hevyStats.workouts[0].location}
                  </span>
                )}
                <span className="rounded-lg border border-[#404040]/10 px-2 py-0.5 text-xs text-[#f5f5f0]/50">
              ...
              {hevyStats?.workouts?.length > 0 ? (
                <>
                  <div className="mb-3 flex flex-wrap items-center gap-3">
                    <span className="rounded-lg border border-[#404040]/10 px-2 py-0.5 font-mono text-xs text-[#f5f5f0]/60">
                    ...
                    <span className="rounded-lg border border-[#404040]/10 px-2 py-0.5 text-xs text-[#f5f5f0]/60">
                      {hevyStats.workouts[0].type || 'Workout'}
                    </span>
                    {hevyStats.workouts[0].notes && (
                      <span className="rounded-lg border border-[#404040]/10 px-2 py-0.5 text-xs italic text-[#f5f5f0]/60">
                        {hevyStats.workouts[0].notes}
                      </span>
                    )}
                  </div>
                      ...
                      <div className="relative h-full w-full">
                        {hevyStats.workouts[0].exercises.map((ex: any, i: number) => {
                          ...
                          return (
                            <div
                              key={i}
                              className={`absolute inset-0 flex min-w-[180px] max-w-full flex-col items-start rounded-2xl border border-[#404040]/40 p-4 transition-all duration-500 ease-in-out ${i === currentExerciseIndex ? 'z-10 scale-100 opacity-100' : 'pointer-events-none z-0 scale-95 opacity-0'}`}
                              aria-hidden={i !== currentExerciseIndex}
                            >
                              <div className="mb-1 flex w-full items-center gap-2">
                                <span
                                  className="flex-1 truncate text-base font-semibold text-[#f5f5f0]"
                                  title={ex.title}
                                >
                                  {ex.title}
                                </span>
                                {ex.superset_id !== null && ex.superset_id !== undefined && (
                                  <span className="ml-2 rounded-lg border border-[#404040]/10 px-2 py-0.5 text-[10px] font-bold text-[#f5f5f0]/80">
                                    Superset {ex.superset_id}
                                  </span>
                                )}
                              </div>
                              <div className="mb-2 mt-4 flex w-full flex-row flex-wrap items-center justify-center gap-6">
                                <span className="text-xs font-bold tracking-wide text-[#f5f5f0]/60">
                                  Sets: <span className="font-mono text-[#f5f5f0]">{totalSets}</span>
                                </span>
                                <span className="text-xs font-bold tracking-wide text-[#f5f5f0]/60">
                                  Reps: <span className="font-mono text-[#f5f5f0]">{totalReps}</span>
                                </span>
                                <span className="text-xs font-bold tracking-wide text-[#f5f5f0]/60">
                                  Volume: <span className="font-mono text-[#f5f5f0]">{totalVolume.toFixed(1)} kg</span>
                                </span>
                              </div>
                              {ex.notes && (
                                <div
                                  className="mt-1 w-full truncate text-xs italic text-[#f5f5f0]/60"
                                  title={ex.notes}
                                >
                                  {ex.notes}
                                </div>
                ...
                <div className="py-4 text-center text-xs text-[#f5f5f0]/60">No recent workout</div>
              )}
            </div>
          </div>
        </div>
