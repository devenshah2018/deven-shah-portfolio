...
import { ExternalLink, Play, Code, Calendar, Users, Award, Zap, ChevronDown, ChevronUp, Github, Globe } from "lucide-react"
...
const projects = [
	...
	},
	{
		...
		sortDate: "2024-08",
		description: "SOC2 compliance platform launched on VSCode Marketplace, accepted into Microsoft for Startups.",
		fullDescription: "Ares is an enterprise-grade security compliance platform that automates SOC2 Type 1 and OWASP compliance checking. Built as a VSCode extension using Rust for performance-critical operations and TypeScript for the user interface. Successfully launched on the VSCode Marketplace and accepted into prestigious accelerator programs including Microsoft for Startups and Buildspace S5.",
		technologies: ["Rust", "TypeScript", "VSCode Extension"],
		allTechnologies: ["Rust", "TypeScript", "VSCode Extension API", "Security Frameworks", "OWASP", "SOC2", "Node.js"],
		type: "link",
	...
	},
	{
		...
		sortDate: "2022-12",
		description: "ML model for cryptocurrency trend forecasting using synthetic data. Presented at SJSU Fall 2022 Expo.",
		fullDescription: "A comprehensive machine learning model designed to forecast cryptocurrency market trends using synthetic data generation and advanced prediction algorithms. The project involved extensive research into time-series analysis, market volatility patterns, and synthetic data generation techniques. Successfully presented at the SJSU Fall 2022 Computer Science Expo as my undergraduate capstone project.",
		technologies: ["Python", "Machine Learning", "Data Science"],
		allTechnologies: ["Python", "TensorFlow", "Pandas", "NumPy", "Scikit-learn", "Matplotlib", "Jupyter", "Time Series Analysis"],
		type: "github",
						...
						{displayedProjects.map((project, index) => {
							return (
								<motion.div
									...
									className="group"
								>
									<Card data-item-id={project.id} className="h-full border border-slate-800 bg-slate-900/50 backdrop-blur-sm transition-all duration-300 gap-0 flex flex-col">
										<CardHeader className="pb-4">
											<div className="flex items-center justify-between mb-4">
												<span className={`text-xs font-semibold px-3 py-1.5 rounded-full border flex items-center gap-2 ${
													project.status === 'Live' ? 'bg-emerald-950/50 border-emerald-700/50 text-emerald-300' :
													project.status === 'In Progress' ? 'bg-blue-950/50 border-blue-700/50 text-blue-300' :
													project.status === 'Completed' ? 'bg-amber-950/50 border-amber-700/50 text-amber-300' :
										...
										<CardContent className="pt-0 flex-1 flex flex-col">
											<div className="space-y-4 flex-1">
												...
												<div className="flex flex-wrap gap-2">
													{(allProjectsExpanded ? project.allTechnologies : project.technologies).map((tech, i) => (
														<Badge key={i} variant="outline" className="bg-slate-800/50 border-slate-700 text-slate-300 text-xs">
															{tech}
														</Badge>
													))}
												</div>
												...
												<ul className="space-y-1">
													{(allProjectsExpanded ? project.achievements : project.highlights).map((item, i) => (
												...
												{allProjectsExpanded && (project as any).metrics && (
													...
													>
														<p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-2">Project Metrics</p>
														<div className="grid grid-cols-2 gap-2">
															{Object.entries((project as any).metrics).map(([key, value]: [string, any]) => (
																<div key={key} className="text-center">
																	<p className="text-lg font-bold text-white">{value}</p>
																	<p className="text-xs text-slate-400 capitalize">{key.replace(/([A-Z])/g, ' $1').toLowerCase()}</p>
																</div>
															))}
														</div>
													</motion.div>
