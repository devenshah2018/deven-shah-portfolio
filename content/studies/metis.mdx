...
The system is containerized using Docker for consistent deployment across environments. Python was chosen for the quantum implementation due to the extensive quantum computing libraries available, including PennyLane for circuit construction and NumPy for numerical operations.
...
METIS is available in two deployment models, providing flexibility for different use cases and user preferences. Both implementations leverage the same underlying quantum-classical hybrid architecture, ensuring consistent performance and capabilities.

### Web Application

The METIS web application provides an interactive interface for quantum-enhanced AutoML, making the technology accessible to users without requiring programming expertise or quantum computing knowledge. The application implements a microservices architecture with four main components: a React frontend for dataset upload and results visualization, a Go API gateway for service coordination, a Python AutoML core for model training and evaluation, and a Python quantum sampler for QAOA-based candidate generation.

Users can upload datasets in various formats (CSV, JSON, Parquet), configure optimization parameters through an intuitive interface, and visualize results including model performance metrics, selected features, and hyperparameter configurations. The web application handles all underlying complexity, from QUBO encoding to quantum circuit execution, presenting results in an accessible format. This deployment model is ideal for interactive exploration, educational purposes, and users who prefer graphical interfaces over programmatic APIs.

### Python Package

METIS is distributed as a production-ready Python package (`metis-automl`) available on PyPI, enabling seamless integration into existing machine learning pipelines and workflows. The package abstracts the underlying quantum and classical components, exposing a straightforward API that requires minimal quantum computing expertise.
...
The core API centers around the `metis.fit()` function, which accepts datasets in multiple formats (CSV, JSON, Parquet file paths or pandas DataFrames) and returns a trained model with optimal hyperparameters and feature selections:
...
print("Validation score:", model.metrics['validation_score'])
```

This dual deployment model ensures flexibility for different use cases, from interactive exploration to programmatic integration in production systems. The Python package makes quantum-enhanced AutoML capabilities accessible to practitioners without requiring deep quantum computing knowledge, while maintaining the full power of the underlying hybrid quantum-classical architecture.
